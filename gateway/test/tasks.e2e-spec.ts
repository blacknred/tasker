...
describe('Tasks (e2e)', () => {
  ...
  afterAll(async () => {
    const { body } = await request(app.getHttpServer()).get('/tasks');
    await Promise.all(
      body.map((t) => request(app.getHttpServer()).delete(`/tasks/${t.id}`)),
    );
    await app.close();
  });

  describe('Create a task', () => {
    it('/users (POST) - should create a user', (done) => {
    ...
    it('/auth (POST) - should create an auth session', (done) => {
      return request(app.getHttpServer())
        .post('/auth')
        .send(userSignupRequestSuccess)
        .expect(201)
  ...
  it('/tasks (GET) - should not retrieve tasks for an unconfirmed user with valid token', (done) => {
    return request(app.getHttpServer())
      .get('/tasks')
      .set('Authorization', userToken)
      .expect(403)
